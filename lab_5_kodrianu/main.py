...
def getData(spark, df):
    ...
    return {"1_average_trip_duration" : get1(df),
        "2_trips_per_day" : get2(df),
        "3_top_start_station_by_month" : get3(spark, df),
        "4_top3_station_for_last_2_week" : get4(df),
        "5_men_or_women_drive_longer_on_average" : get5(df),
        "6_top10_age_by_longest_and_shortest_trip" : get6(df) }
...
def main():
    ...
    for root, dirs, files in os.walk("data"):
        zip_paths.extend([os.path.join(root, file) for file in files if file.endswith(".zip")])
